...
## Part 6: Labeling Anomalies for Training

### scripts/label_anomalies.py

Simple labeling based on known anomaly scenarios:

```python
import pandas as pd
from datetime import datetime, timedelta

def label_anomalies(ocsf_events_path):
    """
    Label anomalies based on patterns generated by load generator.
    ...
    # Rule 1: High latency (> 2 seconds)
    df.loc[df['duration'] > 2000, 'is_anomaly'] = 1

    # Rule 2: 5xx errors
    df.loc[df['status_id'] == 2, 'is_anomaly'] = 1

    # Rule 3: Sudden spike in error rate (rolling window)
    df['error_rate_1min'] = df.set_index('time_dt')['status_id'].rolling('1min').mean()
    df.loc[df['error_rate_1min'] > 0.2, 'is_anomaly'] = 1

    # Rule 4: Memory pressure indicators
    if 'memory_usage_mb' in df.columns:
        df.loc[df['memory_usage_mb'] > df['memory_usage_mb'].quantile(0.95), 'is_anomaly'] = 1

    anomaly_count = df['is_anomaly'].sum()
    total = len(df)

    print(f"Labeled {anomaly_count} / {total} events as anomalies ({anomaly_count/total*100:.2f}%)")

    # Save labeled dataset
    output_path = ocsf_events_path.replace('.parquet', '_labeled.parquet')
...
if __name__ == '__main__':
    label_anomalies('/data/ocsf_logs.parquet')
    label_anomalies('/data/ocsf_metrics.parquet')
```

---

## Part 7: Using Generated Data in Tutorial

Now you can use the generated data in Part 2 (TabularResNet) and Part 3 (Feature Engineering):

```python
# In Part 3: Feature Engineering
import pandas as pd

# Load your generated OCSF data
ocsf_df = pd.read_parquet('data/ocsf_logs_labeled.parquet')
...
Generate realistic observability data in OCSF format for training anomaly detection models.

## What This Generates

- 2 hours of realistic observability data
- ~72,000 normal events + 6-8 anomaly scenarios
- Logs, metrics, and traces in OCSF format
- Pre-labeled anomalies for supervised learning
...
docker-compose up load-generator
python scripts/convert_to_ocsf.py
python scripts/label_anomalies.py
```

## Output Datasets

- `data/ocsf_logs_labeled.parquet` - Application logs with anomaly labels
- `data/ocsf_metrics_labeled.parquet` - System metrics with anomaly labels
- `data/ocsf_traces_labeled.parquet` - Distributed traces with anomaly labels

## Use Cases

- Training TabularResNet models (see [Embedding Anomaly Detection Tutorial](link))
- Testing anomaly detection algorithms
- Benchmarking observability systems
...
This appendix provides a complete, open-source solution for generating realistic observability training data:

1. **Docker Compose stack**: Multi-service application with observability
2. **Instrumented services**: Generate logs, metrics, traces
3. **Load generator**: Creates normal traffic + anomaly scenarios
4. **OCSF converter**: Standardizes data to OCSF format
5. **Anomaly labeling**: Automatic labeling based on patterns
6. **Ready for tutorial**: Outputs Parquet files for Parts 2-9

**To use in the tutorial series**: Add links from Part 3 (Feature Engineering) and Part 4 (Self-Supervised Training) pointing to this appendix for readers who need training data.
